...
class VOC(BaseDataset):
    ...
    def __init__(self, base_dir, split, transforms=None, to_tensor=None):
        ...
        self._inst_dir = os.path.join(self._base_dir, 'SegmentationObjectAug') #实体分割 12031张
        self._scribble_dir = os.path.join(self._base_dir, 'ScribbleAugAuto') # 若监督绘画 12031张，
        self._id_dir = os.path.join(self._base_dir, 'ImageSets', 'Segmentation') #trainaug.txt提供相同类的图片，总共20类（20个txt），同一类在同一个txt
    ...
    def __getitem__(self, idx):
        ...
        image_t = torch.from_numpy(np.array(sample['image']).transpose(2, 0, 1))  # plt.imshow()中则是[H, W, Channels]pytorch中是[CHW]
        # Transform to tensor
